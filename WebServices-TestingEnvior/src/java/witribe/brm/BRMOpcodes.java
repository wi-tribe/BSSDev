...
public class BRMOpcodes extends PortalManager 
{
    ...
    //    System.out.println(" "+username+" "+password+" "+server+" "+port);
    }
...
//         if (outflist.hasField(FldResults.getInst()) == false)
//        {
//            System.out.println("**********");
//            //outflist == null;
//        }
    ...
    public FList GET_ACC_INFO(String cnic,String TransID )
    {
        ...
        FList output = new FList();
           ...
           // if(output.hasField(FldResults.getInst()) == false)
             if(output.hasField(FldResults.getInst()) == false)   
             {
          //     System.out.println("123******************" + output.asString());
               output = null;
            } 
          //  output.dump();
            if(output != null)
    ...
    // new bill payment one link ////
    public FList Bill_Payment(FList input)
    {
        FList output= new FList();
        loggs.smsLogger("TransID: ",", FUNCTION:Bill_Payment() OPcode:BILL_COLLECT, input flist: ",input.toString());
        openContext();
        output = execute(PortalOp.BILL_COLLECT, input);
        loggs.smsLogger("TransID: ",", FUNCTION:Bill_Payment() OPcode:BILL_COLLECT, output flist: ",output.toString());
        closeContext();
        return output;
    }
    ...
    public FList Creat_OBJ(FList input)
    {
        FList output = new FList();
        loggs.smsLogger("TransID: ",", FUNCTION:Creat_OBJ() OPcode:CREATE_OBJ, input flist: ",input.toString());
        openContext();
        output = execute(PortalOp.CREATE_OBJ, input);
        loggs.smsLogger("TransID: ",", FUNCTION:Creat_OBJ() OPcode:CREATE_OBJ, output flist: ",output.toString());
        closeContext();
        return  output;
    }
    ...
    {    
        try {
            ...
            input.set(FldArgs.getInst(), args);
            loggs.smsLogger("TransID:"+TransID,", FUNCTION:searchDeviceID(), OPcode:B searchDeviceID, input flist:\r\n ",input.toString());
            openContext();
            /* Execute the status (active) opcode */
            FList output = execute(PortalOp.SEARCH, input);
            closeContext();
            if (output != null)
            loggs.smsLogger("TransID:"+TransID,", FUNCTION:searchDeviceID(), OPcode:B searchDeviceID, output flist:\r\n ", output.toString());
            //output.dump();
