				...
				<table width="100%">
                     ...
                     foreach($result['subscriptionIds'] as  $profile)
                     {
                         ...
                         switch($profile)
                         {
							...
							case 'Payment_Reminder_Redirect': 
								...
								break;
							case 'Quota_Redirect':
								$prof = 'Qouta Redirection';
								$qouta_check =true;
								break;
                                                        case 'payngo_payment_redirect':
                                                                $prof = 'PaynGO Payment Redirect';
								$remove_check =true;
                                                            break;
							default:
							 $prof = $profile;
                    ...
                    ?>
                        <tr class="textboxBur">
                            ...
                            <? } else if($qouta_check && ($username ==strtolower('PKAliLiaquat') || $username ==strtolower('zafariqbal') || $username ==strtolower('pkSHasnainRaza') || $username ==strtolower('PKMoienA') )  )
								{ ?>
									<td align="left"><input type="button" value="remove" class="button" onclick="javascript:removeProfile('<?=$mac?>','<?=$profile?>');";/></td>
							<?	} ?>
                        </tr>
					...
					<tr class="textboxBur">
                            <td></td>
                            <td align="left"><input type="button" value="refresh" class="button" onclick="javascript:clearSession('<?=$mac?>');"/></td>
                        </tr>
        		</table>
         	</td>
