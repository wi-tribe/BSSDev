...
class Voucher {
    ...
    public function getVouchStatus($vouch_serial)
    {
        ...
        $query = " SELECT VIMS_ITEM_INFORMATION.VOUCHER_SERIAL, VIMS_ITEM_INFORMATION.VOUCHER_DENOMINATION,
                        ...
                        decode(state_id,1,'Active',2,'Used',3,'Expired',8,'Blocked',5,'Stolen',6,'Lost',7,'Damaged') as vouhcer_status
                        FROM
                       ...
                       LEFT OUTER JOIN USERS  TRACKER_ACTION ON (TRACKER_ACTION.USER_ID=VIMS_TRACKER.ACTION_BY)
                        WHERE
                        ( lower(DEVICE_T.device_id) =lower('".$vouch_serial."'))
	...
	public function getReceiptReport($filters)
	{
			...
			if($cse_id!="all" and $cse_id!=null) 
				$condition.= " and cse_id = '$cse_id'";
                 ...
                 $excludeRefund='';
                 if($start_date < '06/20/2011' && $end_date < '06/20/2011')
                 {
                     $excludeRefund = "and payment_method not like 'Cash Refund'"; 
                 }
...
//                       "."$excludeRefund"."
                      ...
                      $query = " select * from (select RECEIPT_ID,RECEIPT_DATE,CUSTOMER_ID,CUSTOMER_TYPE,REGION_ID,SHOP_ID
                                ,CSE_ID,CSE_NAME,decode(channel_type,'1','Direct Sale','2','Shop-Sales Executive','3','BD-Business Development','4','Kiosks','5','BO','6','NBO','7','CSO') as channel_type
                                ,TRANSACTION_ID,PAYMENT_METHOD,INSTRUMENT_NUMBER,INSTRUMENT_BANK
                                ,RECEIPT_AMOUNT,RECEIPT_MODE,RECEIPT_TYPE
                                ,CASE
                                 WHEN receipt_date < to_date('06/12/2011','MM/DD/YYYY') and payment_method = 'Cash Refund'
                                    THEN ('1')
                                ELSE '0'
	...
	public function getReceiptSummaryReport($filters)
	{
		...
		if($cse_id!="all" and $cse_id!=null) 
			$condition.= " and cse_id = '$cse_id'";
                ...
                $query="  select receipts.cse_id as cse_id,receipts.cse_name as cse_name,receipts.Cash,receipts.Cheque,receipts.CreditCard,NVL(vouch.grossvalue,0) as SalesCollection,receipts.Cash_refund CashRefund,SUM(Cash+Cheque+CreditCard+Cash_refund+nvl(grossvalue,0)) as GrossAmnt
                            from
                            (select cse_id,cse_name,round(Cash,2) Cash,round(Cheque,2) Cheque,round(CreditCard,2) CreditCard,round(Cash_refund,2) Cash_refund
				...
				sum(decode(payment_method,'Cash Payment',receipt_amount,0)) Cash,
					...
					from (	SELECT * from ".$receipt_t." where 1=1 $condition $excludeRefund
							UNION ALL
							select * from ".$reversal_t." where 1=1 $condition $excludeRefund 
						)
					where To_date(to_char(receipt_date,'MM/DD/YYYY'),'MM/DD/YYYY') >= to_date('".$start_date."','MM/DD/YYYY')
					and To_date(to_char(receipt_date,'MM/DD/YYYY'),'MM/DD/YYYY') <= to_date('".$end_date."','MM/DD/YYYY')
					group by cse_id, cse_name)
                              group by cse_id,cse_name,Cash,Cheque,CreditCard,Cash_refund) receipts
                              LEFT OUTER JOIN
					...
					To_date(to_char(VIMS_TRACKER.action_date,'MM/DD/YYYY'),'MM/DD/YYYY'),
							...
							vims_item_information.voucher_denomination)
                                  ...
                                  group by receipts.cse_id, receipts.cse_name, receipts.Cash, receipts.Cheque, receipts.CreditCard,receipts.Cash_refund, NVL(vouch.grossvalue,0) ";
	...
	public function getVoucherCollectionReport($filters)
	{
		...
		$query = "select * from (   select VO.ORDER_ID,VO.ORDER_DATE,U.FIRST_NAME||' '||U.LAST_NAME as ORDER_BY,U.USER_ID as SALES_REP_ID,U.FIRST_NAME||' '||U.LAST_NAME as SALES_REP_NAME,
	 ...
	 /*
		...
		*/
			 ...
			 $data[3]['payment_method']= 'Cash Payment';
                         $data[3]['payment_method']= 'Cash Refund';
