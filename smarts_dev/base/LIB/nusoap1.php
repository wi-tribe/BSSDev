...
class nusoap_base {
	...
	*/
	function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded',$soapval=false) {
		...
		if (is_numeric($name)) {
			///saad bashir dar : changed value "_numeric_" to "arg0" for cnic webservice
			$name = 'arg' . $name;
		} elseif (! $name) {
			$name = 'noname';
		}
		...
		if ($phpType == 'struct') {
			if (isset($typeDef['typeClass']) && $typeDef['typeClass'] == 'element') {
				...
				if (isset($typeDef['form']) && ($typeDef['form'] == 'qualified')) {
					...
					//Saad Bashir - changed this for unqualified
					$elementNS = " xmlns=\"$ns\"";
					//$elementNS = " xmlns=\"\""; //actual code
				}
			} else {
				$elementName = $name;
				if ($unqualified) {
			...
			//print htmlspecialchars($xml, ENT_QUOTES) ."_<br>";
