...
class gacl {
	...
	*/
	...
	var $_db_host = '10.1.67.201';

	/** @var string The database user name */
	var $_db_user = 'smarts2'; 

	/** @var string The database user password */
	var $_db_password = 'smarts321';

	/** @var string The database name */
	var $_db_name = 'pindb';

        var $db_tns="(DESCRIPTION =
						(ADDRESS_LIST =
	...
	function gacl($options = NULL) {
		...
		if (is_object($this->_db)) {
			...
			/********************* Database Connectivity *******************************/
                    ...
                    $this->db = ADONewConnection($this->_db_type);
                    $this->db->PConnect($this->db_tns, $this->_db_user,$this->_db_password);
       ...
       /************************************************** Custom Query **********************************************/
        ...
        public function getSectionValue($person)
        {
                $rs = $this->db->Execute("select * from ".$this->_db_table_prefix."ARO where VALUE = '$person'");
                $arr = $rs->FetchRow();
                return $arr;
        }

        public function getModule($function)
       {
                $rs = $this->db->Execute("select * from ".$this->_db_table_prefix."ACO where NAME = '$function'");
                $arr = $rs->FetchRow();
                return $arr;
        }

        public function getaccesslevelAXO($person,$reportname,$arosection)
        {
            $query = "select aco_map.acl_id as acl_id,
                aco_map.value as aco,
                aco_map.section_value as aco_section,
                aro_map.section_value as aro_section,
                aro_map.value as aro,
                aro_group.value as aro_group,
                axo_map.section_value as axo_section,
                axo_map.value as axo

                from acl_aco_map aco_map
                left outer join acl_acl acl on aco_map.acl_id = acl.id 
                left outer join acl_aro_map aro_map on aro_map.acl_id = acl.id
                full outer join acl_aro_groups_map aro_group_map on acl.id = aro_group_map.acl_id
                full outer join acl_aro_groups aro_group on aro_group_map.group_id = aro_group.id
                left outer join acl_axo_map axo_map on acl.id = axo_map.acl_id

                where aco_map.value like '$reportname' and
                (
                (aro_map.value like '$person' and aro_group.value is null)
                or (aro_group.value in ('$arosection') and aro_map.value is null)
                or (aro_group_map.group_id IN (select parent_id from acl_aro_groups WHERE acl_aro_groups.value in ('$arosection')
                and aro_map.value is null and aro_group.value not in ('$arosection') )) ) and acl.allow = 1 and acl.enabled=1";
        ...
        public function getparentGroup($AROsectionvalue)
        {
            $query = "select * from acl_aro_groups
                      where id IN (
                                   select parent_id
                                   from
                                    acl_aro_groups aro_group
                                    where aro_group.value IN ('$AROsectionvalue'))";
            //print($query);
            $rs = $this->db->Execute($query);
                if ($rs)
                while ($arr = $rs->FetchRow())
        ...
        {
            $query = "select * from users where lower(username) = lower('".$loginInfo['username']."') and password = '".$loginInfo['password']."' ";
	...
	*/
	function acl_query($aco_section_value, $aco_value, $aro_section_value, $aro_value, $axo_section_value=NULL, $axo_value=NULL, $root_aro_group=NULL, $root_axo_group=NULL, $debug=NULL) {
		...
		if (!$retarr) {
			...
			$query .= '
					WHERE		a.enabled=1
						AND		(lower(ac.section_value) = lower('. $this->db->quote($aco_section_value) .') AND lower(ac.value) = lower('. $this->db->quote($aco_value) .'))';
			...
			if ($aro_section_value == $this->_group_switch) {
				...
				$query .= '
						AND		((lower(ar.section_value) = lower('. $this->db->quote($aro_section_value) .') AND lower(ar.value) = lower('. $this->db->quote($aro_value) .'))';
	...
	function acl_get_groups($section_value, $value, $root_group=NULL, $group_type='ARO') {
		...
		if (!$retarr) {
			...
			if ($section_value == $this->_group_switch) {
				...
				$where = '
					WHERE		(lower(o.section_value) = lower('. $this->db->quote($section_value) .') AND lower(o.value)= lower('. $this->db->quote($value) .'))
			...
			if ( $root_group != '') {
				...
				$where .= '
						AND		lower(g3.value) = lower('. $this->db->quote( $root_group ) .')
