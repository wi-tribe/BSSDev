...
switch ($_GET['action']) {
	case 'Search':
		...
		if (count($exploded_value_search_str) > 1) {
			$query .= 'lower(value) IN (lower('. implode(',', $exploded_value_search_str) .'))';
		} else {
			$query .= 'lower(value) LIKE lower(' . $db->qstr($value_search_str).')';
		}
		
		$query .= 'OR ';
		
		if (count($exploded_name_search_str) > 1) {
			$query .= 'lower(name) IN (lower('. implode(',', $exploded_name_search_str) .'))';
		} else {
			$query .= 'lower(name) LIKE lower(' . $db->qstr($name_search_str).')';
		}
